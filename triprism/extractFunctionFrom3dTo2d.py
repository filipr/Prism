...
#     """
#         ex = ExtractFunctionTo2d(func3d, func3d,
#             boundary='bottom', elem_facet='top')
#         ex.solve()
#     """
...
#         
#         We hope that:
#         self.fs_2d.finat_element.space_dimension() = len(nodes) 
#         """              
#         
#         self.input_3d = input_3d
...
# #         elem_horizontal = self.fs_3d.ufl_element().horizontal                 
...
#             
#         if elem_facet == 'average':
#             fs_2d_space_dimension = len(nodes) / 2  # out_nodes in Thetis 
#         else:
#             fs_2d_space_dimension = len(nodes)
#          # number of nodes in vertical direction
#         n_vert_nodes = self.fs_3d.finat_element.space_dimension() / \
#                         fs_2d_space_dimension
# 
#         self.idx = op2.Global(len(nodes), nodes, dtype=np.int32, name='node_idx')
#         if elem_facet == 'average':
#             # compute average of top and bottom elem nodes
# 
#             self.kernel = op2.Kernel("""
#                 void my_kernel(double **funcConst, double **func3d, int *idx) {
#                     int nnodes = %(nodes)d;
#                     for ( int d = 0; d < nnodes; d++ ) {
#                         for ( int c = 0; c < %(func_dim)d; c++ ) {
#                             for ( int e = 0; e < %(v_nodes)d; e++ ) {
#                                 funcConst[idx[d]+e][c] = 0.5*(func3d[idx[d]][c] + func3d[idx[d + nnodes]][c]);
#                             }
#                         }
#                     }
#                 }""" % {'nodes': fs_2d_space_dimension,
#                         'func_dim': self.fs_3d.value_size, 
...
#                         for ( int c = 0; c < %(func_dim)d; c++ ) {
#                             for ( int e = 0; e < %(v_nodes)d; e++ ) {
#                                 funcConst[idx[d]+e][c] = func3d[idx[d]][c];
#                             }
#                         }
#                     }
#                 }""" % {'nodes': fs_2d_space_dimension,
...
# 
#     def solve(self):
#         with timed_stage('set 3d constant in time'):
#             # execute par loop
#             op2.par_loop(self.kernel, self.fs_3d.mesh().cell_set,
